...
- The root `LICENSE` is the source of truth and every plugin directory carries a committed copy of it, because each wheel and sdist must ship the license text. The conventions check fails on any plugin `LICENSE` that is a symlink, missing, or not byte-identical to the root file (`cp LICENSE python/<name>/LICENSE` to refresh); `pyproject.toml` declares `license = "MIT"` and `license-files = ["LICENSE"]`; `check_wheel.py` verifies the packaged text equals the root file.
...
- Tests self-provision the Temporal dev server (`WorkflowEnvironment.start_local`, version pinned in `tests/__init__.py`) with its default configuration; add a `--dynamic-config-value` flag in the template conftest only when a test needs a server feature that is off by default.
