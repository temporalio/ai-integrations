...
- Local-only divergence: the OpenAI Agents MCP v2 adapter was proposed in sdk-python PR #1793 but was never merged into sdk-python's default branch. It was added directly to this repository as ordinary local work, not as an import or re-sync; it must not receive the `history-import` label or an `IMPORTS.md` row. It touches ten otherwise-imported files: `python/openai_agents/README.md`, `src/temporalio/contrib/openai_agents/{_mcp.py, _mcp_backend.py, _openai_runner.py, _temporal_mcp_server.py, _temporal_openai_agents.py, testing.py, workflow.py}` and `tests/contrib/openai_agents/{test_mcp_v2.py, test_openai.py}`. `upstream` stays set because everything else in the plugin still re-syncs from sdk-python main. Future re-syncs merge on top of this local divergence; resolve conflicts in these files by preserving both the upstream changes and the adapter, never by taking upstream's version wholesale. Fixes to the adapter itself land here directly.
