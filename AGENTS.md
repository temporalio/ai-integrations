...
- One `LICENSE` at the root. Each plugin has a committed relative symlink `LICENSE -> ../../LICENSE`, never a copy; `pyproject.toml` declares `license = "MIT"` and `license-files = ["LICENSE"]`. Windows checkouts without symlink support get a stub file, which is why release builds run only on Linux and `check_wheel.py` verifies the wheel's LICENSE equals the root file.
...
- Tests self-provision the Temporal dev server (`WorkflowEnvironment.start_local`, version pinned in `tests/__init__.py`). MCP tests spawn `npx`, so Node must be present (it is on GitHub-hosted runners).
- Offline tests: `tests/conftest.py` marks every test with pytest-recording's `vcr` marker; HTTP traffic is replayed from `tests/contrib/<name>/cassettes/<module>/<test>.yaml` with body-inclusive matching and `allow_playback_repeats=True`. If `OPENAI_API_KEY` is unset it is set to the dummy `sk-cassette-replay` so upstream skip guards do not skip; the Agents SDK's default backend trace exporter is removed with `set_trace_processors([])` (do not use `OPENAI_AGENTS_DISABLE_TRACING`, it breaks the tracing tests). `OFFLINE_SKIPS` in the conftest lists tests that cannot replay. A `CannotOverwriteExistingCassetteException` means a test made a request with no cassette: run `OPENAI_API_KEY=<real key> make record` locally (never in CI; `make record` refuses when `CI` is set) and commit the cassette. Cassettes must contain no secrets; the conventions check scans for them.
...
- Required checks on `main`: `ci-status`, the org CODEOWNERS workflow (automatic), `license/cla` once the CLA app is installed, `opengrep/scan` after a week green.
...
```bash
cd python/<name> && make sync && make lint && make test && make test-time-skipping && make build
