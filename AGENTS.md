...
- Tooling: ruff, pyright, basedpyright, mypy (`mypy_path = "src"`, `explicit_package_bases`), pydocstyle (google), pytest + xdist (`-n auto --dist=worksteal`; the `os._exit(0)` hook is xdist-aware). `python/_shared/` owns their composable non-Python defaults (`python.mk`, `pyrightconfig.json`, `mypy.ini`, `ruff.toml`, `pydocstyle.ini`); plugin-specific settings remain in `pyproject.toml`. All tools are invoked through `make` targets; see `make help`.
