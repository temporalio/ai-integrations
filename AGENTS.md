...
- Layout: `python/<name>/{pyproject.toml, uv.lock, plugin.toml, Makefile, README.md, LICENSE, src/temporalio/contrib/<name>/, tests/}`. Plugins that still name an upstream keep its test tree (`tests/contrib/<name>/...`, `tests/helpers/`, `tests/conftest.py`) so re-syncs never conflict. A plugin may retain that historical layout after ownership transfers here; flatten it separately after cutover.
...
- Provenance guard (`python/_shared/temporalio_ai_integrations_support.py`, mirrored by `scripts/ci/smoke.py`) runs at every pytest session start and fails loudly if the install is editable, any file differs from the distribution's RECORD, files under the package directory are not owned by the distribution, or another distribution ships the same paths. While `plugin.toml` `[release] allow-final = false`, the SDK's overlap (`temporalio<=1.32` ships `temporalio/contrib/openai_agents/*`) is tolerated with a warning. `tests/test_installed_matches_source.py` additionally byte-compares the installed package with `src/`.
...
- Tests self-provision the Temporal dev server (`WorkflowEnvironment.start_local`, version pinned in `python/_shared/temporalio_ai_integrations_pytest.py`) with its default configuration; add a `--dynamic-config-value` flag in a plugin's conftest only when one of its tests needs a server feature that is off by default.
- Test scaffolding: `python/_shared/` is its own locked uv tooling project and owns the standard provenance guard, local dev-server fixtures and pytest hooks. The repository conventions job runs its Ruff, Mypy and BasedPyright checks once from that environment; each plugin tests the fixtures at runtime but does not type-check them again. Each plugin loads the pytest plugin from the conftest generated by `python/_template/tests/conftest.py.tmpl`; plugin-specific fixtures stay in that plugin's conftest. Tests must not require real provider credentials; use deterministic local models, mock transports, or in-process servers to exercise provider behavior in CI.
...
1. Open a release PR that sets `version` in `pyproject.toml` (re-sync first only if `plugin.toml` still names an upstream). Merge it.
...
A plugin is upstream-managed only while its `plugin.toml` names an `upstream`. During that period,
fix code and tests upstream, merge there, and re-sync with a `history-import` PR. Once the field is
removed, this repository is the source of truth and code and tests are edited here normally.
`python/openai_agents` has completed that ownership transfer and is not re-synced from `sdk-python`.

## Transition rules (until the SDK cutover PR merges)

- `python/openai_agents` is owned here even before SDK cutover; make its code and test changes in this repository. Temporary cutover-only changes carry `TRANSITION(sdk-cutover):` in the code comment or commit message.
...
cd python/_shared && make sync && make lint
```
