...
2. Do not edit imported files (`src/...`, `tests/contrib/<plugin>/...`) while the plugin's `plugin.toml` still names an `upstream`. Fix upstream, then re-sync. The one recorded exception is the `openai_agents` MCP v2 adapter (Transition rules below).
...
- Recorded exception: the OpenAI Agents MCP v2 adapter was developed in sdk-python PR #1793, which was closed unmerged on 2026-09-03 when the MCP work moved here (`python/mcp`). Its seven commits were migrated with `Migrated-From` trailers (`scripts/migrate/IMPORTS.md`, 2026-09-03 row) and exist only in this repository. They touch ten imported files: `python/openai_agents/README.md`, `src/temporalio/contrib/openai_agents/{_mcp.py, _mcp_backend.py, _openai_runner.py, _temporal_mcp_server.py, _temporal_openai_agents.py, testing.py, workflow.py}` and `tests/contrib/openai_agents/{test_mcp_v2.py, test_openai.py}`. `upstream` stays set: everything else in the plugin still re-syncs from sdk-python main, and a re-sync merges on top of these commits (resolve conflicts in those files by keeping both sides; never take upstream's version wholesale). Fixes to the adapter itself land here directly. The "not a descendant of the previous import" determinism rule in `scripts/migrate/README.md` did not require a full re-import because the shared filtered history stayed byte-identical (merge base `ca5c201` on `main`).
